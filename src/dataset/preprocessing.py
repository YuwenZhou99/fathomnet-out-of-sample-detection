...
def prepare_dataloaders(general_cfg: dict[str, Any]) -> tuple[DataLoader, DataLoader, dict[int, str]]:
    ...
    train_ds = FathomNetMultiLabelDataset(train_df, cat_map, general_cfg['image_dir'], general_cfg['img_ext'],
                                          transform=train_tf)

    # not sure if val should have augmentations
    val_ds = FathomNetMultiLabelDataset(val_df, cat_map, general_cfg['image_dir'], general_cfg['img_ext'],
                                        transform=val_tf)
